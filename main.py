...
if __name__ == "__main__":
    try:
        ...
        """
        for i, url in enumerate(products_urls):  # 2をproduct_urlsに変える、iをurlに変える
            price_url_maxpages = get_review_pages(url)

            # 感想が0件のページ
            if price_url_maxpages[0][2] == 0:
                break

            with open('urls.csv', 'a', newline='') as f:
                writer = csv.writer(f)
                writer.writerows(price_url_maxpages)
